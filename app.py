...
def load_gsc_data(queries_bytes, pages_bytes):
    ...
    # Normalisation colonnes — supporte FR et EN
    col_map_q = {}
    for c in queries_df.columns:
        cl = c.lower().strip()
        if any(k in cl for k in ['query', 'requête', 'requete', 'fréquentes', 'frequentes', 'mot-clé', 'mot cle']):
            col_map_q[c] = 'Query'
        elif any(k in cl for k in ['clic', 'click']):
            col_map_q[c] = 'Clicks'
    ...
    for c in pages_df.columns:
        cl = c.lower().strip()
        if any(k in cl for k in ['page', 'url', 'landing', 'populaire', 'popular', 'adresse']):
            col_map_p[c] = 'Page'
        elif any(k in cl for k in ['clic', 'click']):
            col_map_p[c] = 'Clicks'
        elif 'impression' in cl:
...
if st.button("🚀 Lancer la génération", type="primary"):
    ...
    # Liste de mots finnois explicites — couvre les cas que langdetect rate
    # (mots courts, mots empruntés, termes marketing finnois)
    FINNISH_WORDS = {
        'liidejä', 'liidi', 'liidit', 'liidien', 'liidejä',
        'yritys', 'yrityksen', 'yritykselle', 'yritykset',
        'palvelu', 'palvelut', 'palveluita', 'palvelujen',
        'tutkimus', 'tutkimukset', 'tutkimuksen',
        'markkinointi', 'markkinoinnin', 'markkinatutkimus',
        'brändi', 'bränditutkimus', 'bränditietoisuus',
        'asiakas', 'asiakaspalvelu', 'asiakastyytyväisyys',
        'asiakaskysely', 'asiakaspalaute', 'asiakaskokemus',
        'myynti', 'myynnin', 'ostaja', 'ostajat',
        'markkina', 'markkinat', 'markkinoilla',
        'kysely', 'kyselytutkimus', 'haastattelu',
        'mittari', 'csat', 'strategia', 'kilpailu',
        'suomi', 'suomessa', 'suomalais', 'helsinki',
        'etusivu', 'toimisto', 'hinnoittelu',
        'kuluttaja', 'kuluttajat', 'kuluttajatutkimus',
        'prospekti', 'outbound', 'inbound',
        'verkkokauppa', 'digitaalinen', 'hyvä', 'laadukas',
        'mystery', 'kvantitatiivinen', 'kvalitatiivinen',
        'virtuaaliassistentti', 'etäassistentti',
    }

    def detect_lang(query):
        """
        Détection de langue hybride :
        1. Si la requête contient un mot finnois connu → FI (règle explicite)
        2. Sinon → langdetect (algorithme statistique)
        Évite les faux positifs estonien/hongrois sur les mots courts finnois.
        """
        q_words = set(str(query).lower().split())
        if q_words & FINNISH_WORDS:
            return 'fi'
        try:
