...
with st.sidebar:
    ...
    serpapi_key = st.text_input(
        ...
        help="Free at serpapi.com (100 searches/month). Detects current P1 competitors dynamically each month."
    )
    if backend_url:
        try:
...
if not backend_url:
    ...
    st.warning("⚠️ Enter your SerpApi key in the sidebar — needed to detect current P1 competitors dynamically.")
        ...
        with st.spinner("Sending files to Colab backend..."):
            try:
                ...
                response = requests.post(
                    ...
                    data={'groq_api_key': groq_api_key, 'serpapi_key': serpapi_key},
                    timeout=30
                )
...
if st.session_state.get('results'):
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
        ...
        "🔑 LSI Terms", "📄 Content Briefs", "🌐 SERP Competitors", "📥 Export"
    ])
    ...
    with tab6:
        st.subheader("🌐 SERP Competitors Analysis")
        st.caption("Top P1 competitors detected dynamically via SerpApi this month")

        top5 = results.get('top5_competitors', [])
        serp_credits = results.get('serp_credits_used', 0)
        if top5:
            st.info(f"**{serp_credits} SerpApi credits used** — Top 5 competitors detected: {', '.join(top5)}")
            serp_sum = results.get('serp_summary', [])
            if serp_sum:
                st.subheader("BERT Similarity — Katrium vs P1 Google")
                st.dataframe(pd.DataFrame(serp_sum), use_container_width=True, hide_index=True)
            serp_det = results.get('serp_details', [])
            if serp_det:
                with st.expander("Full SERP Details"):
                    st.dataframe(pd.DataFrame(serp_det), use_container_width=True, hide_index=True)
        else:
