...
with st.sidebar:
    ...
    test_mode = st.checkbox(
        ...
        value=False,
        help="Runs SerpApi on only 5 queries — uses 5 credits. Use this to verify competitors and scraping before full analysis."
    )
    if test_mode:
        st.markdown('''<div style="background:rgba(227,179,65,0.08);border:1px solid rgba(227,179,65,0.2);
                    border-radius:8px;padding:0.6rem 0.9rem;color:#e3b341;font-size:0.78rem;margin-top:0.3rem;">
            ⚠️ Test mode — only 5 SerpApi credits used.<br>
            Verify competitors are correct before full run.
        </div>''', unsafe_allow_html=True)
    else:
        st.markdown('<div style="color:#6b7ea3;font-size:0.74rem;margin-top:0.2rem;">Full analysis — uses all MR cluster credits</div>', unsafe_allow_html=True)
