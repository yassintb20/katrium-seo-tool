...
with st.sidebar:
    st.markdown("""
    <div style="display:flex;align-items:center;gap:0.6rem;"><div style="background:linear-gradient(135deg,#1d6fcc,#58a6ff);border-radius:8px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:1rem;font-weight:800;color:white;font-family:Space Grotesk,sans-serif;flex-shrink:0;">K</div><div class="sidebar-logo" style="margin:0;">Katrium SEO</div></div>
    ...
    st.markdown('<div style="color:#8ba3c7;font-size:0.78rem;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:0.5rem;">Analysis Mode</div>', unsafe_allow_html=True)

    analysis_mode = st.radio(
        "Analysis Mode",
        options=["🗓️ This Month (use saved SERP data)", "🔄 New Month (run SerpApi)"],
        index=0,
        help="This Month = use saved CSV data, no SerpApi credits used. New Month = run SerpApi dynamically.",
        label_visibility="collapsed"
    )

    use_saved_serp = (analysis_mode == "🗓️ This Month (use saved SERP data)")

    if use_saved_serp:
        st.markdown('''<div style="background:rgba(86,211,100,0.07);border:1px solid rgba(86,211,100,0.2);
                    border-radius:8px;padding:0.7rem 0.9rem;color:#7ee787;font-size:0.78rem;margin-bottom:0.5rem;">
            ✅ No SerpApi credits used — upload your saved SERP CSV files below.
        </div>''', unsafe_allow_html=True)
        serp_details_file = st.file_uploader(
            "📄 Katrium_SERP_Details.csv",
            type=["csv"],
            help="SERP details from previous month analysis"
        )
        serp_sim_file = st.file_uploader(
            "📄 Katrium_SERP_Similarity.csv",
            type=["csv"],
            help="SERP similarity scores from previous month analysis"
        )
    else:
        serp_details_file = None
        serp_sim_file     = None
        st.markdown('''<div style="background:rgba(227,179,65,0.08);border:1px solid rgba(227,179,65,0.2);
                    border-radius:8px;padding:0.7rem 0.9rem;color:#e3b341;font-size:0.78rem;margin-bottom:0.5rem;">
            ⚠️ SerpApi credits will be used — all MR cluster queries will be analyzed.
        </div>''', unsafe_allow_html=True)

        test_mode = st.checkbox(
            "🧪 Test mode (5 queries only)",
            value=False,
            help="Uses only 5 SerpApi credits to verify results before full run."
        )
        if test_mode:
            st.markdown('<div style="color:#e3b341;font-size:0.74rem;margin-top:-0.3rem;">⚠️ Only 5 credits used — good for verification</div>', unsafe_allow_html=True)
    
    if not use_saved_serp and not "test_mode" in dir():
        test_mode = False
    ...
    <div style="background:rgba(86,211,100,0.07);border:1px solid rgba(86,211,100,0.2);
                border-radius:8px;padding:0.6rem 0.9rem;color:#7ee787;font-size:0.78rem;">
        🌍 <strong>Competitor Detection</strong><br>
        <span style="color:#6b7ea3;">Global results (no geo bias) — detects real B2B Market Research leaders worldwide</span>
    </div>
    ''', unsafe_allow_html=True)
    gl_code = None  # pas de gl — résultats globaux
    ...
    if run_clicked:
        with st.spinner("Sending files to backend…"):
            try:
                ...
                if use_saved_serp and serp_details_file and serp_sim_file:
                    serp_details_file.seek(0)
                    serp_sim_file.seek(0)
                    files_to_send['serp_details'] = ('serp_details.csv', serp_details_file.read(), 'text/csv')
                    files_to_send['serp_sim']     = ('serp_sim.csv',     serp_sim_file.read(),     'text/csv')
                ...
                response = requests.post(
                    ...
                    data={
                        ...
                        'test_mode':       'true' if (not use_saved_serp and test_mode) else 'false',
                        'use_saved_serp':  'true' if use_saved_serp else 'false'
                    },
                    timeout=30
                )
...
if st.session_state.get('results'):
    ...
    with tab1:
        ...
        st.markdown('<div class="section-sub">Ranked by <strong>Priority Score</strong> = CPS × log(impressions) × 1/position (0-100).<br><span style="color:#ff7b72;">🔴 CRITIQUE ≥75</span> · <span style="color:#e3b341;">🟠 HAUTE ≥50</span> · <span style="color:#e6edf3;">🟡 MOYENNE ≥25</span> · <span style="color:#56d364;">🟢 BASSE &lt;25</span><br><strong>CPS</strong> = commercial intent score via BERT centroids (0=informational, 1=commercial).<br><strong>Action:</strong> Start with 🔴 CRITIQUE — optimize the page title/meta/H2, or create a new page if it is a GAP.</div>', unsafe_allow_html=True)
        ...
        if qw:
            ...
            st.markdown('<div class="section-sub">Pages ranking in <strong>positions 10–20</strong> with 100+ impressions/month — already indexed by Google, just need a push.<br>These are the <strong>fastest wins</strong>: a small title/meta optimization can jump from page 2 to page 1 within weeks.<br><strong>Action:</strong> Use the New Title and New Meta recommendations → update directly in WordPress SEO plugin (Yoast or RankMath).</div>', unsafe_allow_html=True)
        ...
        if new_opps:
            ...
            st.markdown('<div class="section-sub">Queries with <strong>commercial intent</strong> (CPS ≥ 0.50) where no Katrium page matches well (Best Fit &lt; 0.70).<br>These represent <strong>revenue opportunities</strong> — users want to hire a market research firm but find no relevant Katrium page.<br><strong>Best Fit</strong> = BERT cosine similarity between the query and the best Katrium URL (0=no match, 1=perfect).<br><strong>Action:</strong> Create a new SERVICE page for each query — use the Content Brief generated in the Content Briefs tab as your blueprint.</div>', unsafe_allow_html=True)
    ...
    with tab2:
        ...
        st.markdown('<div class="section-sub">Pages with <strong>0 clicks but 100+ impressions</strong> — Google indexes them but users never click, or the page is broken/missing.<br><strong>Action:</strong> Recreate missing pages · Fix broken redirects · Add content to empty pages. Each dead page is losing real traffic every single day.</div>', unsafe_allow_html=True)
    ...
    with tab3:
        ...
        st.markdown('<div class="section-sub">Pages with <strong>0 clicks</strong> competing for the same keyword as a stronger page — consolidated via 301 redirect.<br>Language detected automatically from URL slug (Finnish/English/Swedish...) → each page redirects to its winner in the <strong>same language</strong>.<br><strong>Action:</strong> In WordPress → use the <em>Redirection</em> plugin → add each row as one 301 rule. Takes 5 minutes.</div>', unsafe_allow_html=True)
    ...
    with tab4:
        ...
        st.markdown('<div class="section-sub"><strong>LSI terms</strong> used heavily by P1 competitors but <strong>missing from katrium.eu</strong> — detected via TF-IDF, filtered by BERT similarity ≥ 0.40 to the MR centroid.<br><strong>GAP Score</strong> = how much more competitors use this term vs Katrium (higher = higher priority).<br><strong>Action:</strong> Add these terms naturally in H2 headings and body content. Target 1-3 terms per page section — never stuff.</div>', unsafe_allow_html=True)
        ...
        if lsi_page:
            ...
            st.markdown('<div class="section-sub">Terms that appear in your <strong>GSC queries</strong> for a page but are <strong>missing from the actual page content</strong> (TF-IDF analysis).<br>Users find your page for these terms but the page does not mention them → low relevance signal → poor ranking.<br><strong>Gap Score</strong> = query TF-IDF minus page TF-IDF (higher = more urgent). <strong>Action:</strong> Add missing terms naturally in H2 headings or FAQ section.</div>', unsafe_allow_html=True)
    ...
    with tab5:
        ...
        st.markdown('<div class="section-sub">Generated by <strong>Groq Llama 3.3 70B</strong> using RAG with real P1 competitor pages + LSI terms.<br><span style="color:#ff7b72;">🆕 NEW PAGE</span> = GAP — no Katrium page exists → create new page in <strong>English</strong> always.<br><span style="color:#58a6ff;">✏️ OPTIMIZE</span> = existing page → optimize title/H1/meta → <strong>language matches the existing page</strong> (Finnish/English).<br><strong>Action:</strong> Copy the slug → create/edit page in WordPress → use H2 structure as content outline → integrate LSI terms naturally.</div>', unsafe_allow_html=True)
    ...
    with tab6:
        ...
        st.markdown('<div class="section-sub">Top 5 domains appearing most often in <strong>Google P1</strong> across all Market Research cluster queries — detected via SerpApi.<br><strong>BERT Similarity</strong> = semantic distance between Katrium page and competitor URL (0=very different, 1=identical).<br><span style="color:#56d364;">🟢 BON ≥0.75</span> = good coverage · <span style="color:#e3b341;">🟡 MOYEN ≥0.55</span> = needs work · <span style="color:#ff7b72;">🔴 FAIBLE &lt;0.55</span> = rewrite needed.<br><strong>Action:</strong> For every 🔴 FAIBLE query → rewrite the Katrium page to match the structure and vocabulary of the P1 competitor shown.</div>', unsafe_allow_html=True)
