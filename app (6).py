...
with st.sidebar:
    ...
    target_market = st.selectbox(
        ...
        options=[
            ("🇫🇮 Finland (fi)", "fi"),
            ("🇪🇺 Europe — UK English (gb)", "gb"),
            ("🌍 International — US English (us)", "us"),
        ],
        format_func=lambda x: x[0],
        index=0,
        help="Defines which Google market SerpApi uses to detect P1 competitors. Finland = primary Katrium market."
    )
    gl_code = target_market[1]
    st.markdown(f'<div style="color:#6b7ea3;font-size:0.74rem;margin-top:-0.3rem;">SerpApi: gl={gl_code} · Competitors from Google {target_market[0]}</div>', unsafe_allow_html=True)
